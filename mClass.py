...
class CameraShiftDetector:
    ...
    def match(self, standardImPath, comparisonImPath, savePath, orignalSavePath, saveFeaturePoints=False):

        try:
            if osp.isfile(standardImPath) and osp.isfile(comparisonImPath):
            ...
            assert self.standardIm.shape == self.comparisonIm.shape
            ...
            if lessthresNumber >= self.minMatchedPoints:
                ...
                self.saveReuslt(savePath, orignalSavePath, matched2CalculateDistance, saveFeaturePoints)
                return "Moved"
            ...
            #########################################################################
            cv2.resize(merged, (int(totalWidth * 0.5), int(maxHeight * 0.5)))
            #########################################################################
            cv2.imwrite(savePath, merged)
            
            return "ShapeError"
    
    def saveReuslt(self, savePath, orignalSavePath, matched2CalculateDistance, saveFeaturePoints=False, saveGid=True):        
        ...
        orignal_result = np.zeros((self.imH, self.imW*2, 3), dtype=np.uint8)
        orignal_result[:self.imH, :self.imW] = self.standardIm
        orignal_result[:self.imH, self.imW:] = self.comparisonIm
        ...
        cv2.imwrite(orignalSavePath, orignal_result)
...
class CameraScreenFaultyDetector:
    ...
    def analyseEdge(self, gray, h, w):
        ...
        if edgeCount / (w * h) > 0.1: # or edgeCount / (w * h) < 0.005:
            flag = True
